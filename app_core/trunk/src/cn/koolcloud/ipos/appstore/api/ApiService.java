...
public class ApiService {
	...
	private static final String HOST = "https://appstore.koolyun.com"; // 生产服务器
 	//private static final String HOST = "http://appstore.koolyun.cn:10000"; // 测试服务器
	//private static final String HOST = "https://appstore.koolyun.cn:10001";// 测试服务器2,
//	private static final String HOST = "https://appstore1.koolyun.com:10001";//临时测试服务器3
	
	private static final String VERSION_VAL = "1.3";
	...
	private static final String GET_APP_DETAIL_ACTION = "getAppDetailsByPN";
	...
	private static final String CHECK_PAYMENT_ACTION = "checkPayment";
	private static final String SYNC_RESPONSE_ACTION = "syncResponse";
	...
	private static final String GET_APP_DETAIL_PATH = "/appstore/app/getAppDetailsByPN";
...
//	private static final String DOWNLOAD_PICTURE_PATH = "/appstore/download/download";
	...
	private static final String GET_CHECK_PAY_MENT = "/appstore/payment/checkPayment";
	private static final String GET_SYNC_RESPONSE = "/appstore/payment/syncResponse";
	...
	*/
	public static JSONObject getDownloadPicJson(String picId, Context ctx) {
		...
		try {
			...
			jsonObj.put(Constants.JSON_KEY_TERMINAL_ID, MySPEdit.getTerminalID(ctx));
...
//			dataJson = reponseJson.getJSONObject(Constants.REQUEST_DATA);
	...
	public static Task checkAllAppUpdate(Context context, String terminalId,
			List<AppInfo> list, cn.koolcloud.ipos.appstore.interfaces.CallBack callBack) {
		JSONObject reqestJson = null;
		// FIXME mod url, request action and arguments
		String url = HOST + REQUEST_CONTEXT + CHECK_APP_UPDATE_PATH;
		try {
			...
			for (AppInfo appInfo : list) {
				JSONObject appObj = new JSONObject();
				appObj.put(Constants.JSON_KEY_PACKAGE_NAME, appInfo.getPackageName());
				appObj.put(Constants.JSON_KEY_VERSION_CODE, appInfo.getVersionCode());
		...
		return AsyncHttpClient.request(context, url, reqestJson, callBack, ResultSet.RESPONSETYPE.RESPONSE_TYPE_JSON_OBJECT);
	}
...
//			jsonObj.put(Constants.JSON_KEY_DOWNLOAD_APK_URL, httpUrl);
	...
	public static Task getAppDetail(Context context, String terminalID, String string,
			CallBack getAppDetailCallBack) {
		if (TextUtils.isEmpty(terminalID)) {
			return null;
		}
		
		JSONObject reqestJson = null;
		String url = HOST + REQUEST_CONTEXT + GET_APP_DETAIL_PATH;
		try {
			reqestJson = generateReq(GET_APP_DETAIL_ACTION);
			reqestJson.put(Constants.JSON_KEY_TERMINAL_ID, terminalID);
			JSONObject jsonObject = new JSONObject(string);
			reqestJson.put(Constants.REQUEST_DATA, jsonObject);
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		return AsyncHttpClient.request(context, url, reqestJson, getAppDetailCallBack,
				ResultSet.RESPONSETYPE.RESPONSE_TYPE_JSON_OBJECT);
	}
	
	public static Task checkPayMent(Context context,String terminalID, String id,CallBack getAppDetailCallBack){
		if (TextUtils.isEmpty(terminalID)) {
			return null;
		}
		JSONObject reqestJson = null;
		String url = HOST + REQUEST_CONTEXT + GET_CHECK_PAY_MENT;
		try {
			reqestJson = generateReq(CHECK_PAYMENT_ACTION);
			reqestJson.put(Constants.JSON_KEY_TERMINAL_ID, terminalID);
			reqestJson.getJSONObject(Constants.REQUEST_DATA).put(
					Constants.JSON_KEY_ID, id);
		} catch (Exception e) {
		...
		//SYNC_RESPONSE_ACTION
		return AsyncHttpClient.request(context, url, reqestJson, getAppDetailCallBack,
				ResultSet.RESPONSETYPE.RESPONSE_TYPE_JSON_OBJECT);
	}
	
	public static Task syncResponse(Context context,String terminalID, String outTrackNo,String payName,String outData,CallBack getAppDetailCallBack){
		if (TextUtils.isEmpty(terminalID)) {
			return null;
		}
		JSONObject reqestJson = null;
		String url = HOST + REQUEST_CONTEXT + GET_SYNC_RESPONSE;
		try {
			reqestJson = generateReq(SYNC_RESPONSE_ACTION);
			reqestJson.put(Constants.JSON_KEY_TERMINAL_ID, terminalID);
			reqestJson.getJSONObject(Constants.REQUEST_DATA).put(
					Constants.JSON_KEY_PAY_OUT_TRADE_NO, outTrackNo).
					put(Constants.JSON_KEY_SYNC_RESPONSE, payName).
					put(Constants.JSON_KEY_PAYMENT_DATA, outData);
		} catch (Exception e) {
			e.printStackTrace();
		}
		MyLog.i("---GeneralAppsListAdapter----null----dd--");
		return AsyncHttpClient.request(context, url, reqestJson, getAppDetailCallBack,
				ResultSet.RESPONSETYPE.RESPONSE_TYPE_JSON_OBJECT);
