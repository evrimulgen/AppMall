...
public class MySPEdit {
	...
	public static final String UPDATE_RECORD							= "appUpdate_record";
	...
	public static int getAppNums(Context context) {
		SharedPreferences prefer = context.getSharedPreferences(PREFS_NAME,
				Context.MODE_PRIVATE);
		return prefer.getInt("AppNums", 1);
	}
	public static void saveAppNums(Context context, int MultiThreadNums) {
		SharedPreferences prefer = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
		prefer.edit().putInt("AppNums", MultiThreadNums).commit();
	}
	
	public static void setUpdateRecordCode(Context context,int versionCode){
		SharedPreferences prefer = context.getSharedPreferences(UPDATE_RECORD, Context.MODE_PRIVATE);
		prefer.edit().putInt("versionCode", versionCode).commit();
	}
	
	public static int getUpdateRecordCode(Context context){
		SharedPreferences prefer = context.getSharedPreferences(UPDATE_RECORD, Context.MODE_PRIVATE);
		return prefer.getInt("versionCode", 0);
	}
	

	public static void setUpdateRecordName(Context context,String versionName){
		SharedPreferences prefer = context.getSharedPreferences(UPDATE_RECORD, Context.MODE_PRIVATE);
		prefer.edit().putString("versionName", versionName).commit();
	}
	
	public static String getUpdateRecordName(Context context){
		SharedPreferences prefer = context.getSharedPreferences(UPDATE_RECORD, Context.MODE_PRIVATE);
