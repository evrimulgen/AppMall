...
public class AppDetailLeftFragment extends BaseFragment implements DownloadEngineCallback {
	...
	private static final int HANDLE_SYNC_RESPONSE = 5;
	public static final int HANDLE_CHECK_PAYMENT = 6;
	...
	private boolean isWrapPay = false;
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(requestCode == HANDLE_CHECK_PAYMENT){
			if(data != null){
				Bundle bundle = data.getExtras();
				boolean result = bundle.getBoolean("result", false);
				MyLog.i("-AppDetailLeftFragment---onActivityResult-----"+requestCode+"===== " + result);
				if(result){
					app.setPay(true);
					isWrapPay = true;
				}else{
					Toast.makeText(getActivity(), bundle.getString("message"), 3000).show();
				}
			}
		}
...
//		}
		...
		if (installedStatus == Constants.APP_NO_INSTALLED_DOWNLOAD) {
			...
			downloadButton.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View arg0) {
					if(Env.isBackground(application, app.getPackageName())){
						Env.goToFront(application, app.getPackageName());
					}else{
	...
	private void changeButtonState(final App appInfo, final Button downloadButton,
			final RelativeLayout downloadRelativeLayout2, final ProgressBar progressBar,
			final TextView downloadText, final boolean isInitState, boolean isUpdateState) {
...
//						}, 1500);
			...
			if(isPay(appInfo)&& !appInfo.isPay()){
				...
				downloadButton.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						if(application.getUserBean().getGradeId().equals(Constants.USRE_GRADE_INCHARGE)){
							
							ApiService.checkPayMent(getActivity(), MySPEdit.getTerminalID(getActivity()), appInfo.getId(), new CallBack() {
								...
								public void onFailure(String msg) {
									...
									ToastUtil.showToast(getActivity(), R.string.back_ground_error);
								}
								...
								public void onSuccess(JSONObject response) {
									...
									if(pay.getType() == Constants.PAY_TYPE_NOT_FREE){
										...
										MyLog.d("----------已经付费的软件----------------"+pay.getList().get(0).getUrl());
										intent.putExtra("url", pay.getList().get(0).getUrl());
										intent.putExtra("position", -2);
										getActivity().startActivityForResult(intent, HANDLE_CHECK_PAYMENT);
									}else if(pay.getType() == Constants.PAY_TYPE_FREE){
										MyLog.d("----------已经付费的软件----------------");
										appInfo.setPay(true);
										initComponents();
										//changeButtonState(appInfo,downloadButton,downloadRelativeLayout2,progressBar,downloadText,false,false);
									}else if(pay.getType() == Constants.PAY_TYPE_PROGRESS){
										ToastUtil.showToast(getActivity(), R.string.pay_in_progress);
									}else{
										ToastUtil.showToast(getActivity(), R.string.back_ground_error);
									}
								}
							...
							ToastUtil.showToast(getActivity(), R.string.user_download_permission_failed);
						}
					}
				});
			...
			// 没有安装，后台也没有在下载这个App
			if(appInfo.getFee() == 0|| appInfo.getFee() >0 &&appInfo.isPay()){
				progressBar.setProgress(0);
	...
	Handler mHandler = new Handler(new Handler.Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			case HANDLE_DOWNLOAD_PROCESS_DONE:
				...
				break;
			case HANDLE_SYNC_RESPONSE:
				Pay pay = (Pay) msg.obj;
				if(pay == null){
					MyLog.i("---GeneralAppsListAdapter----null------" +pay.getList().size());
				}
				ApiService.syncResponse(getActivity(), MySPEdit.getTerminalID(getActivity()), pay.getList().get(0).getOutTradeNo(),
