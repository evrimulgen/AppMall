...
import cn.koolcloud.ipos.appstore.AppCommentDialogActivity;
import cn.koolcloud.ipos.appstore.BigImageActivity;
...
public class AppDetailMainFragment extends BaseFragment implements View.OnClickListener {
...
//		snapImageContainer = (LinearLayout) getActivity().findViewById(R.id.sw_main_gallery_container);
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		new Handler().postDelayed(new Runnable() {
			@Override
			public void run() {
				LinearLayout.LayoutParams params = (LayoutParams) imagesGallery.getLayoutParams();
				params.width = (int) (rootScrollView.getWidth() * 0.618);
				params.height = params.width * 240 / 400;
				imagesGallery.setLayoutParams(params);
			}
		}, 500);
	}
	...
	public void getAppDetails() {
		ApiService.getAppDetailsByIds(application, MySPEdit.getTerminalID(application), 
	...
	private CallBack getAppDetailsCallBack = new CallBack() {
		...
		public void onSuccess(JSONObject jsonObj) {
			try {
				MyLog.d("-------getAppsInfo=" + jsonObj.toString());
	...
	public void getCommentSummary() {
		ApiService.getCommentSummary(application, MySPEdit.getTerminalID(application), 
	...
	private CallBack getCommentSummaryCallBack = new CallBack() {
		...
		public void onSuccess(JSONObject jsonObj) {
			try {
				MyLog.d("-------getCommentSummary=" + jsonObj.toString());
	...
	private void setGallerySelectListener() {
		...
		imagesGallery.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				...
				intent.putExtra("url", snapShortList.get(position % size));
				getActivity().startActivity(intent);
			}
		});
	}
	
	final Handler autoGalleryHandler = new Handler(new Handler.Callback() {
		@Override
	...
	public void onClick(View view) {
		...
		case R.id.sw_main_comment_add_bt:
			Intent intent = new Intent(getActivity(), AppCommentDialogActivity.class);
		...
		case R.id.more_comment:
			AppDetailCommentFragment commentFragment = AppDetailCommentFragment.getInstance();
