	...
	public class ImplMSCService extends IMSCService.Stub {
        ...
        public ParcelableApp checkUpdate(String packageName, int versionCode) throws RemoteException {
        	...
        	if(!TextUtils.isEmpty(terminalId)){
        		...
        		boolean isOpenPush = MySPEdit.getPushNotificationTag(getApplication());
        		if (isOpenPush) {
        		...
        		while(MySPEdit.getTerminalID(getApplicationContext()).trim().length() == 0){
        			try {			
						Thread.sleep(100);
						MyLog.i("---MSCService--wait bai du push-");
					} catch (Exception e) {
						e.printStackTrace();
					}
        		}
        		terminalId = MySPEdit.getTerminalID(getApplicationContext());
        		JSONObject appObj = ApiService.checkSoftUpdate(terminalId, packageName, versionCode);
	        	ParcelableApp app = JsonUtils.parseJSONParcelApp(appObj);
	        	return app;
        	}
