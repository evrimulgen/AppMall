...
import cn.koolcloud.ipos.appstore.NewsPushDialogActivity;
			...
			try {
				...
				if (customJson != null && !customJson.isNull(Constants.JSON_KEY_TYPE)) {
					...
					if (type == Constants.TYPE_APP_UPDATE) {
						...
						Intent intent = new Intent(context,NewsPushDialogActivity.class);
						intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
						Bundle bundle = new Bundle();
						pushMessage = customContentString;
						bundle.putString(Constants.SER_KEY, customContentString);
		...
		public void onSuccess(JSONObject jsonObj) {
			try {
				...
				MySPEdit.saveTerminaID(ctx, terminalId);
 					//Constants.loginFalg = true;
				MyLog.i("--------------------reciver push----------------------------------"+MySPEdit.getTerminalID(ctx));
			} catch (Exception e) {
