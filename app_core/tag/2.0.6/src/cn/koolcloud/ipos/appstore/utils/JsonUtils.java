...
import cn.koolcloud.ipos.appstore.entity.PayMents;
...
public class JsonUtils {
	...
	*/
	...
	public static double getDoubleValue(JSONObject jsonObject,String jsonNameKey) throws JSONException{
		if (null == jsonObject) {
			return -1.00;
		}
		if (!jsonObject.isNull(jsonNameKey)) {
			return jsonObject.getDouble(jsonNameKey);
		}
		return -1.00;
	}
			...
			if (categoriesJson != null && categoriesJson.length() > 0) {
				...
				for (int i = 0; i < categoriesJson.length(); i++) {
					...
					Category category = new Category(getStringValue(categoryJson, Constants.JSON_KEY_ID),
							...
							getStringValue(categoryJson, Constants.JSON_KEY_DESC),
	...
	*/
			...
			if (categoriesJson != null && categoriesJson.length() > 0) {
				...
				for (int i = 0; i < categoriesJson.length(); i++) {
					...
					for (int j = 0; j < appLength; j++) {
						...
						app.setFee(getDoubleValue(appJson,Constants.JSON_KEY_FEE));
						appsList.add(app);
						
						HashMap<String, String> tempMap = new HashMap<String, String>();
						tempMap.put(app.getDownloadId(), app.getName());
						MainActivity.notifMap.put(app.getPackageName(), tempMap);
					}
	 ...
	 * @return
	 */
	public static List<App> parseJSONAppDetails(JSONObject jsonObject) {
		List<App> appsList = new ArrayList<App>();
		try {
			JSONArray appsJson = getJSONObject(jsonObject,
					Constants.REQUEST_DATA).getJSONArray("apps");
	...
	public static Pay parsePayJson(JSONObject jsonObject){
		Pay pay = new Pay();
		List<PayMents> list = null;
		try {
			JSONObject dataJson = getJSONObject(jsonObject, Constants.REQUEST_DATA);
			pay.setType(getIntValue(dataJson, Constants.JSON_KEY_PAY_TYPE));
			MyLog.i("JsonUtils-----------" + pay.getType());
			JSONArray paymentArray =getJSONArray(dataJson,Constants.JSON_KEY_PAY_MENTS);
			if(paymentArray != null && paymentArray.length() >0){
				list = new ArrayList<PayMents>();
				for(int i=0;i<paymentArray.length();i++){
					JSONObject objectJson = paymentArray.getJSONObject(i);
					PayMents payMents = new PayMents();
					 
					payMents.setName(getStringValue(objectJson, Constants.JSON_KEY_PAY_NAME));
					payMents.setOutTradeNo(getStringValue(objectJson, Constants.JSON_KEY_PAY_OUT_TRADE_NO));
					if(pay.getType() == Constants.PAY_TYPE_NOT_FREE){
						payMents.setUrl(getStringValue(objectJson, Constants.JSON_KEY_PAY_URL));
					}else if(pay.getType() == Constants.PAY_TYPE_PROGRESS){
						payMents.setBody(getStringValue(objectJson, Constants.JSON_KEY_PAY_BODY));
					}
					payMents.setSubject(getStringValue(objectJson, Constants.JSON_KEY_PAY_SUBJECT));
					payMents.setFee(getDoubleValue(objectJson, Constants.JSON_KEY_PAY_FEE));
					list.add(payMents);
