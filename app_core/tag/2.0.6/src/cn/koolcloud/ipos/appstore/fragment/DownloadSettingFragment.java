...
public class DownloadSettingFragment extends BaseFragment implements View.OnClickListener {
	...
	private ImageButton addAppImageButton;
	private ImageButton minusAppImageButton;
	private TextView threadNumText;
	private TextView appNumText;
	...
	private boolean deleteApkTag = true;
	...
	private void initViews() {
		...
		addAppImageButton = (ImageButton) v.findViewById(R.id.addAppImageButton);
		addAppImageButton.setOnClickListener(this);
		minusAppImageButton = (ImageButton) v.findViewById(R.id.minusAppImageButton);
		minusAppImageButton.setOnClickListener(this);
		appNumText = (TextView) v.findViewById(R.id.appNumText);
		appNumText.setText(""+MySPEdit.getAppNums(getActivity()));
		
		deleteApkButtonlayout = (LinearLayout) v.findViewById(R.id.deleteApkButtonlayout);
		deleteApkButtonlayout.setOnClickListener(this);
		deleteApkImageView = (ImageView) v.findViewById(R.id.deleteApkImageView);
		switchImageStatus();
	...
	public void onClick(View view) {
		...
		case R.id.minusThreadImageButton:
			...
			threadNumText.setText("" + MySPEdit.getMultiThreadNums(getActivity()));
			break;
		case R.id.addAppImageButton:
			if(MySPEdit.getAppNums(getActivity()) >= 3) {
				ToastUtil.showToast(getActivity(), R.string.app_down_top_num);
				return;
			}
			MySPEdit.saveAppNums(getActivity(), MySPEdit.getAppNums(getActivity())+1);
			appNumText.setText("" + MySPEdit.getAppNums(getActivity()));
			break;
		case R.id.minusAppImageButton:
			if(MySPEdit.getAppNums(getActivity()) <= 1) {
				ToastUtil.showToast(getActivity(), R.string.app_down_low_num);
				return;
			}
			MySPEdit.saveAppNums(getActivity(), MySPEdit.getAppNums(getActivity())-1);
			appNumText.setText("" + MySPEdit.getAppNums(getActivity()));
