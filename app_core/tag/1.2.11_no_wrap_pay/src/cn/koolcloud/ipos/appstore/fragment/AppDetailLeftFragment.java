...
public class AppDetailLeftFragment extends BaseFragment implements DownloadEngineCallback {
	...
	private static final int HANDLE_CHECK_PAYMENT = 6;
											...
											public void run() {
												...
												if(array != null){
													if(array[0].contains(Constants.REQUEST_DEAL_FINISH)){
														//	appInfo.setPay(true);
														mHandler.obtainMessage(HANDLE_CHECK_PAYMENT, pay).sendToTarget();
													}else{
														Looper.prepare();
														ToastUtil.showToast(getActivity(), R.string.deal_not_finished, true);
														Looper.loop();
												...
												MyLog.d(result);
												mHandler.obtainMessage(HANDLE_SYNC_RESPONSE, pay).sendToTarget();
											}
										}.start();
...
//						}, 1500);
	...
	public void getCheckBackPoll(){
 		//while(!isPayFlag){
	 		ApiService.checkPayMent(getActivity(), MySPEdit.getTerminalID(getActivity()), app.getId(), new CallBack() {
